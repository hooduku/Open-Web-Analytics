...
class owa_db extends owa_base {
	...
	function generateSelectQuerySql() {
		...
		$limit_param = $this->_fetchSqlParams('limit');
		$offset_param = $this->_fetchSqlParams('offset');
		if(!empty($limit_param)) {
	
		if($limit_param == 0)
		    $limit_param = 1;
      if($offset_param == '' || $offset_param == 0)
          $offset_param = 1; //sometimes only LIMIT is set in the calling program
		$top_string = "TOP ".$limit_param*$offset_param."  "; //only select the TOP rows for the given pages
 	   //further slicing is done in function paginated_Result set below

       } //not empty limit_param
	...
	function _makeLimitClause() {
	   ...
	   $ret = '';
	   return $ret;  //handled as TOP and slice LIMIT is not there in sqlserver yet
	...
	function _query() {
		...
		switch($this->_sqlParams['query_type']) {
			...
			case 'select':
				...
				$ret = $this->get_paginated_results($ret);
	...
	function createTable($entity) {
	   ...
	   if('OWA_DB_TYPE' == 'mysql')
		return $this->query(sprintf(OWA_SQL_CREATE_TABLE, $entity->getTableName(), $columns, $table_options));
		else
		  {
			 $table_name = $entity->getTableName();
   	     $owa_sqlsrv_create_table_query = $this->owa_sqlsrv_create_table($table_name,$columns,$table_options);
   			 if($owa_sqlsrv_create_table_query === true)
	     		     return true;
		        return $this->query($owa_sqlsrv_create_table_query);
		  }
	...
	function owa_sqlsrv_create_table($x,$y) {
	$table_exists = $this->get_table_exists($x);
	if($table_exists == 1)
	    return true; //no need to create table check for values later
	else
	 $table_create_query = "CREATE TABLE $x ($y)"; //Should NOT DROP TABLE all stored values will be gone
	return $table_create_query;
	}
	
	function get_table_exists($x) {
	$select_query = "SELECT * FROM SYSOBJECTS WHERE NAME = '$x' AND TYPE IN ('U','N')";
	$x = $this->get_row($select_query);
	$y = $this->getAffectedRows();
   return $y;
	}
	
	function get_paginated_results($incoming_resultset) {
	//print_r($incoming_resultset);
	$limit_par = 0;
	$offset_par = 0;
   $limit_par = (int)$this->_fetchSqlParams('limit');
	$offset_par = (int)$this->_fetchSqlParams('offset');
	
   	if($limit_par == '' || $limit_par == 0)
		    return $incoming_resultset; // no limit set on this query

      if($offset_par == '' || $offset_par == 0)
          $offset_param = 0;
      else
          ;
          
      $array_start_param = $offset_par; //number of elements to skip
      $array_length_param = $limit_par; //number of elements to consider
      
      $outgoing_resultset = @array_slice($incoming_resultset,$array_start_param,$array_length_param);
      
      //print_r($outgoing_resultset);
    	return $outgoing_resultset;
	}
